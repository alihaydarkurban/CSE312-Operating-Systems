...
li $t2, 25 # It is for counter of number from 1 to 25. (Including 1 and 25) 
	
# This loop works if $t2 > 0. It is for counting of collatz's number.
outer_loop:
	beq $t2, 0, terminate_program # Jump the terminate_program label to terminate the program
...
inner_loop:
	...
	# This part works when the number become 1
	li $v0, 4
	la $a0, new_line # Printing new_line
	syscall
	
	addi $t2, $t2, -1 # Decreasing the number counter.
	j outer_loop
###############################################################################################
