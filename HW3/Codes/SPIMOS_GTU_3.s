.data
name_of_the_kernel: .asciiz "==============================This is SPIMOS_GTU_3==============================\n"
...
main:
###############################################################################################	
	li $t2, 1 # Counter for adding 3 program for each program (excluding init)
	...
	addi $a3, $zero, 10 # There are 10 process
...
###############################################################################################	
	...
	move $t0, $v0 # Move to random integer to $t0 
	move $t5, $v0 # Copy the $t0

	# This is for choosing random program and saving the address of it
	jal choose_random_program

	# New different integer calculation
	add $t0, $t5, $zero # Copy the old value of $t0 that is $t5 to the $t0
	addi $t0, $t0, 3 	# Adding 3 to the calculate new random number different from the old ones.
	move $t5, $t0		# Copy the $t0
	rem $t0, $t0, 4		# Module operation of the $t0. It makes the $t0 in the range.
...
# ProcessTable struct for LinearSearch
	...
	lw $t0, program_counter # The initial program_counter
	sw $t0, 128($s0) 		# Store the program_counter after the registers
	move $t0, $t4 			# The ProcessId for LinearSearch
	sw $t0, 132($s0) 		# Store the ProcessId after the program_counter
	...
	# Counter operation
	addi $t4, $t4, 1 # This is for process_id
	addi $t2, $t2, 1 # This is counter to creating new process (Adding 3 same programs)
	j program_0
###############################################################################################	
...
# ProcessTable struct for BinarySearch
	...
	lw $t0, program_counter # The initial program_counter
	sw $t0, 128($s0) 		# Store the program_counter after the registers
	move $t0, $t4 			# The ProcessId for BinarySearch
	sw $t0, 132($s0) 		# Store the ProcessId after the program_counter
...
# ProcessTable struct for Collatz
	...
	lw $t0, program_counter # The initial program_counter
	sw $t0, 128($s0) 		# Store the program_counter after the registers
	move $t0, $t4 			# The ProcessId for Collatz
	sw $t0, 132($s0) 		# Store the ProcessId after the program_counter
...
# ProcessTable struct for Palindrome
	...
	lw $t0, program_counter # The initial program_counter
	sw $t0, 128($s0) 		# Store the program_counter after the registers
	move $t0, $t4			# The ProcessId for Palindrome
	sw $t0, 132($s0) 		# Store the ProcessId after the program_counter
